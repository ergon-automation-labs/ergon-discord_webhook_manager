# Getting Started with bot_army_discord_webhook_manager

This guide walks you through setting up Discord Webhook Manager for local development.
...
DATABASE_NAME=bot_army_discord_webhook_manager_dev
```
...
Discord Webhook Manager uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_discord_webhook_manager/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_discord_webhook_manager/skills/example.ex lib/bot_army_discord_webhook_manager/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyDiscordWebhookManager.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_discord_webhook_manager.command.classify"]
...
```elixir
# In lib/bot_army_discord_webhook_manager/application.ex or similar

defmodule BotArmyDiscordWebhookManager.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyDiscordWebhookManager.Skills.Classify,
      BotArmyDiscordWebhookManager.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_discord_webhook_manager/skills/my_skill.ex
...
This creates a release named `discord_webhook_manager` that is deployed to `/opt/ergon/releases/discord_webhook_manager/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/discord_webhook_manager/`
