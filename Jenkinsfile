pipeline {
  ...
  environment {
    BOT_NAME = 'discord_webhook_manager'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-discord-webhook-manager"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/discord_webhook_manager/bin/discord_webhook_manager"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyDiscordWebhookManager.Release.create()'"
          $RELEASE_BIN eval 'BotArmyDiscordWebhookManager.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyDiscordWebhookManager.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyDiscordWebhookManager.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_discord_webhook_manager/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_discord_webhook_manager/releases/start_erl.data)
        fi
